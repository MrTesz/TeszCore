...
public class AsyncMariaDBManager extends AbstractMariaDBManager implements AsyncDBManager, Copyable<AsyncMariaDBManager> {
    ...
    public CompletableFuture<Void> createOrAlter(@NotNull DBTable dbTable) throws IllegalArgumentException {
        ...
        return CompletableFuture.runAsync(() -> {
            try (Connection conn = getConnection(); Statement statement = conn.createStatement()) {
                ...
                TeszCoreApi.getInstance().getLogger(DebugLevel.LEVEL8, projectName).debug("Created table " + name + " if not exists in " + (System.currentTimeMillis() - start) + " ms");
                ...
                .whenComplete(
                        (result, ex) -> {
                            if (ex == null)
                                TeszCoreApi.getInstance().getLogger(DebugLevel.LEVEL0, projectName).warning("Async executeSql task of AsyncMariaDBManager " + getName() + " of Project " + projectName
    ...
    public CompletableFuture<SelectionResults> executeSelect(@NotNull String sql, @NotNull List<Object> sqlParams, @NotNull String tableName) {
        ...
        return CompletableFuture.supplyAsync(() -> {
            ...
            return new SelectionResults(returnValue);
                ...
                (result, ex) -> {
                    if (ex == null)
                        TeszCoreApi.getInstance().getLogger(DebugLevel.LEVEL0, projectName).warning("Async executeSelect task of AsyncMariaDBManager " + getName() + " of Project " + projectName
                                + " timed out after " + timeoutSeconds + " seconds.");
                }
        );
    }
    ...
    public CompletableFuture<Boolean> columnExists(@NotNull String tableName, @NotNull String columnName) {
        ...
        return CompletableFuture.supplyAsync(
                ...
                (result, ex) -> {
                    if (ex == null)
                        TeszCoreApi.getInstance().getLogger(DebugLevel.LEVEL0, projectName).warning("Async columnExists task of AsyncMariaDBManager " + getName() + " of Project " + projectName
                                + " timed out after " + timeoutSeconds + " seconds.");
                }
        );
    }
    ...
    public CompletableFuture<Boolean> indexExists(@NotNull String tableName, @NotNull String indexName) {
        ...
        return CompletableFuture.supplyAsync(
                ...
                (result, ex) -> {
                    if (ex == null)
                        TeszCoreApi.getInstance().getLogger(DebugLevel.LEVEL0, projectName).warning("Async indexExists task of AsyncMariaDBManager " + getName() + " of Project " + projectName
                                + " timed out after " + timeoutSeconds + " seconds.");
                }
        );
    }
