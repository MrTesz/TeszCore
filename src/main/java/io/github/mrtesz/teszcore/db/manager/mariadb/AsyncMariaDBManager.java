...
public class AsyncMariaDBManager extends AbstractMariaDBManager implements AsyncDBManager {
    ...
    @Override
    public CompletableFuture<int[]> executeBatchSql(@NotNull List<BatchSqlStatement> sqlStatements, @NotNull String tableName) {
        long start = System.currentTimeMillis();
    ...
    public CompletableFuture<Boolean> indexExists(@NotNull String tableName, @NotNull String indexName) {
        ...
        return CompletableFuture.supplyAsync(
                ...
                (result, ex) -> {
                    if (ex == null)
                        TeszCoreApi.getInstance().getLogger(DebugLevel.LEVEL1, projectName).error(
                                "Async indexExists task of AsyncMariaDBManager " + getName() + " of Project " + projectName + " timed out after " + timeoutSeconds + " seconds."
                        );
                }
        );
    }
